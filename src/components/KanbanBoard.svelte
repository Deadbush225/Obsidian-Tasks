<script lang="ts">
	...
	function computeColCards(
		...
		for (const c of cards) {
			const status = (
				c.id in overrides ? overrides[c.id] : c.status
			) as TaskStatus;
					...
					>
						...
						{#if card.isSubtask}
							<div class="card-parent-label">
								<span class="parent-dot" style="background:{card.accentColor}"
								></span>
								{card.parentTitle}
							</div>
						{/if}
						...
						<div class="card-header">
							...
							<span
								...
								tabindex="0"
								on:keydown={(e) =>
									e.key === "Enter" && onOpenTask(card.filePath)}
								>{card.title}</span
							>
							<span
								class="priority-badge"
								style="background:{priorityColors[card.priority] ?? '#888'}"
							>
						...
						{#if card.subtaskCount > 0}
							<div class="card-subtasks">
								<span class="subtask-count"
									>{card.subtaskDone}/{card.subtaskCount} subtasks</span
								>
								<div class="subtask-progress">
									<div
										class="subtask-fill"
										style="width:{(card.subtaskDone / card.subtaskCount) *
											100}%"
									></div>
								</div>
							</div>
						...
						<div class="card-footer">
							{#if !card.isSubtask}
								...
								<button
									...
									on:click|stopPropagation={() =>
										onAddSubtask(card.id, card.title)}
							...
							<button
								class="btn-archive"
								on:click|stopPropagation={() =>
									onArchiveTask(card.id, card.filePath, card.isSubtask)}
