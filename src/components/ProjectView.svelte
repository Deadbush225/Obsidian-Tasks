<script lang="ts">
	...
	/** Called when the user clicks the Sync button in the toolbar. */
	export let onSync: (() => Promise<void>) | null = null;
	...
	async function handleSync() {
		...
		try {
			...
			syncState = "error";
			syncLabel = e?.message ?? "Sync failed";
		} finally {
		...
		<div class="project-selector">
			...
			{#if liveProjects.length === 0}
				<span class="no-projects"
					>No projects found in your projects folder.</span
				>
			{/if}
		</div>
			...
			{#if onSync}
				...
				>
					{#if syncState === "syncing"}
						<span class="sync-icon spinning">↻</span> Syncing…
	...
	<div class="view-container">
		{#if !currentProject}
			<div class="empty-state">
				...
				<p>
					No project selected. Create a folder inside your configured projects
		...
		{:else if viewMode === "gantt"}
			<GanttChart
				...
				onAddSubtask={(parentId, parentTitle) =>
					openNewTaskModal(parentId, parentTitle)}
