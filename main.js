...
function reconcile(state2, array, anchor, flags2, get_key) {
	...
	if (is_animated) {
		for (i = 0; i < length; i += 1) {
			value = array[i];
			key = get_key(value, i);
			effect2 = /** @type {EachItem} */ items.get(key).e;
...
class GanttSettingTab extends obsidian.PluginSettingTab {
	...
	display() {
		...
		new obsidian.Setting(containerEl)
			.setName("Server URL")
			.setDesc(
				"Base URL of your sync backend, e.g. https://yourname.heliohost.us/obgantt/backend",
			)
			.addText((text) =>
				text
		...
		new obsidian.Setting(containerEl)
			...
			.addButton((btn) =>
				btn
					...
					.setCta()
					.onClick(async () => {
						const { syncBaseUrl, syncEmail, syncPassword } =
							this.plugin.settings;
						if (!syncBaseUrl || !syncEmail || !syncPassword) {
							syncStatusEl.textContent =
								"⚠️  Fill in Server URL, Email, and Password first.";
							return;
						}
						btn.setDisabled(true).setButtonText("Syncing…");
						syncStatusEl.textContent = "";
						try {
							const svc = new GanttSyncService(
								syncBaseUrl,
								syncEmail,
								syncPassword,
							);
							await svc.login();
							const count = await svc.syncAll(
								this.plugin.app,
								this.plugin.settings.projectsFolder,
							);
							syncStatusEl.textContent = `✅  Sync complete — ${count} task(s) updated.`;
						} catch (e) {
...
async function loadProjects(app, projectsFolder) {
	...
	for (const child2 of rootFolder.children) {
		...
		const tasks = await loadTasksFromFolder(
			app,
			projectFolder,
			projectFolder.path,
		);
...
var root_4$2 = /* @__PURE__ */ from_html(
	`<span class="tag svelte-x7v2jt"> </span>`,
);
var root_3$3 = /* @__PURE__ */ from_html(
	`<div class="card-tags svelte-x7v2jt"></div>`,
);
var root_5$1 = /* @__PURE__ */ from_html(
	`<div class="card-subtasks svelte-x7v2jt"><span class="subtask-count svelte-x7v2jt"> </span> <div class="subtask-progress svelte-x7v2jt"><div class="subtask-fill svelte-x7v2jt"></div></div></div>`,
);
var root_7$1 = /* @__PURE__ */ from_html(`<span> </span>`);
var root_8$1 = /* @__PURE__ */ from_html(`<span> </span>`);
var root_6$1 = /* @__PURE__ */ from_html(
	`<div class="card-dates svelte-x7v2jt"><!> <!></div>`,
);
var root_10$1 = /* @__PURE__ */ from_html(
	`<div class="subtask-item svelte-x7v2jt" role="button" tabindex="0"><span>●</span> <span> </span></div>`,
);
var root_9$1 = /* @__PURE__ */ from_html(
	`<details class="subtask-list svelte-x7v2jt"><summary class="svelte-x7v2jt">Subtasks</summary> <!></details>`,
);
var root_2$2 = /* @__PURE__ */ from_html(
	`<div draggable="true"><div class="card-header svelte-x7v2jt"><span class="card-title svelte-x7v2jt" role="button" tabindex="0"> </span> <span class="priority-badge svelte-x7v2jt"> </span></div> <!> <!> <!> <!></div>`,
);
var root_11$1 = /* @__PURE__ */ from_html(
	`<div class="kanban-empty svelte-x7v2jt">Drop tasks here</div>`,
);
var root_1$3 = /* @__PURE__ */ from_html(
	`<div role="list"><div class="kanban-col-header svelte-x7v2jt"><span class="col-title svelte-x7v2jt"> </span> <span class="col-count svelte-x7v2jt"> </span></div> <div class="kanban-cards svelte-x7v2jt"><!> <!></div></div>`,
);
var root$3 = /* @__PURE__ */ from_html(
	`<div class="kanban-board svelte-x7v2jt"></div>`,
);
	...
	each(
		...
		($$anchor2, col) => {
				...
				($$anchor3, task) => {
					...
					var node_1 = sibling(div_5, 2);
					{
						var consequent = ($$anchor4) => {
							...
							each(
								div_6,
								5,
								() => (get(task), untrack(() => get(task).tags)),
								index,
								($$anchor5, tag) => {
									var span_4 = root_4$2();
						...
						if_block(node_1, ($$render) => {
							if ((get(task), untrack(() => get(task).tags.length > 0)))
								$$render(consequent);
						});
					}
					var node_2 = sibling(node_1, 2);
					{
						var consequent_1 = ($$anchor4) => {
							...
							template_effect(
								($0, $1) => {
									set_text(
										text_5,
										`${$0 ?? ""}/${(get(task), untrack(() => get(task).subtasks.length)) ?? ""} subtasks`,
									);
								...
								[
									() => (
										...
										untrack(
											() =>
												get(task).subtasks.filter((s) => s.status === "done")
													.length,
										)
									),
									() => (
						...
						if_block(node_2, ($$render) => {
							if ((get(task), untrack(() => get(task).subtasks.length > 0)))
								$$render(consequent_1);
						});
					}
					var node_3 = sibling(node_2, 2);
					{
							...
							var node_4 = child(div_10);
							{
								var consequent_2 = ($$anchor5) => {
									...
									template_effect(() =>
										set_text(
											text_6,
											`📅 ${(get(task), untrack(() => get(task).startDate)) ?? ""}`,
										),
									);
									append($$anchor5, span_6);
								};
								if_block(node_4, ($$render) => {
									if ((get(task), untrack(() => get(task).startDate)))
										$$render(consequent_2);
								});
							}
							var node_5 = sibling(node_4, 2);
							{
								var consequent_3 = ($$anchor5) => {
									...
									template_effect(() =>
										set_text(
											text_7,
											`→ ${(get(task), untrack(() => get(task).endDate)) ?? ""}`,
										),
									);
									append($$anchor5, span_7);
								};
								if_block(node_5, ($$render) => {
									if ((get(task), untrack(() => get(task).endDate)))
										$$render(consequent_3);
								});
							}
						...
						if_block(node_3, ($$render) => {
							if (
								(get(task),
								untrack(() => get(task).startDate || get(task).endDate))
							)
								$$render(consequent_4);
						});
					}
					var node_6 = sibling(node_3, 2);
					{
						var consequent_5 = ($$anchor4) => {
							...
							each(
								node_7,
								1,
								() => (get(task), untrack(() => get(task).subtasks)),
								index,
								($$anchor5, sub) => {
									...
									template_effect(() => {
										set_class(
											span_8,
											1,
											`subtask-status status-${(get(sub), untrack(() => get(sub).status)) ?? ""}`,
											"svelte-x7v2jt",
										);
									...
									event(
										"keydown",
										div_11,
										(e) => e.key === "Enter" && onOpenTask()(get(sub).filePath),
									);
					...
					template_effect(
						($0) => {
							classes_1 = set_class(
								div_4,
								1,
								"kanban-card svelte-x7v2jt",
								null,
								classes_1,
								{ dragging: get(draggingId) === get(task).id },
							);
							set_text(text_2, (get(task), untrack(() => get(task).title)));
							set_style(
								span_3,
								`background:${(get(task), untrack(() => priorityColors[get(task).priority] ?? "#888")) ?? ""}`,
							);
							set_text(text_3, $0);
						},
						[
							() => (
								get(task),
								untrack(() => priorityLabel(get(task).priority))
							),
						],
					);
					...
					event(
						"keydown",
						span_2,
						(e) => e.key === "Enter" && onOpenTask()(get(task).filePath),
					);
			...
			var node_8 = sibling(node, 2);
			{
				...
				var d = /* @__PURE__ */ user_derived(
					() => (
						get(col),
						untrack(() => getTasksForColumn(get(col).id).length === 0)
					),
				);
			...
			template_effect(
				($0) => {
					classes = set_class(
						div_1,
						1,
						"kanban-column svelte-x7v2jt",
						null,
						classes,
						{ "drag-over": get(dragOverCol) === get(col).id },
					);
					set_style(
						div_2,
						`border-top: 3px solid ${(get(col), untrack(() => get(col).color)) ?? ""}`,
					);
				...
				[
					() => (
						get(col),
						untrack(() => getTasksForColumn(get(col).id).length)
					),
				],
			);
...
var root_3$2 = /* @__PURE__ */ from_html(
	`<button class="expand-btn svelte-152mm6m" aria-label="Toggle subtasks"> </button>`,
);
var root_4$1 = /* @__PURE__ */ from_html(
	`<span class="expand-placeholder svelte-152mm6m"></span>`,
);
var root_5 = /* @__PURE__ */ from_html(
	`<span class="expand-placeholder subtask-indent svelte-152mm6m"></span>`,
);
var root_1$2 = /* @__PURE__ */ from_html(
	`<div><!> <span class="gantt-task-link svelte-152mm6m" role="link" tabindex="0"> </span> <span class="status-dot svelte-152mm6m"></span></div>`,
);
var root_6 = /* @__PURE__ */ from_html(
	`<div class="gantt-month-cell svelte-152mm6m"> </div>`,
);
var root_7 = /* @__PURE__ */ from_html(`<div> </div>`);
var root_8 = /* @__PURE__ */ from_html(
	`<div class="today-line svelte-152mm6m"></div>`,
);
var root_10 = /* @__PURE__ */ from_html(
	`<div role="button" tabindex="-1" aria-label="Set date"></div>`,
);
var root_11 = /* @__PURE__ */ from_html(
	`<div class="gantt-bar svelte-152mm6m"><div class="bar-handle bar-handle-left svelte-152mm6m"></div> <span class="bar-label svelte-152mm6m"> </span>  <div class="bar-handle bar-handle-right svelte-152mm6m"></div></div>`,
);
var root_9 = /* @__PURE__ */ from_html(
	`<div class="gantt-grid-row svelte-152mm6m"><!> <!></div>`,
);
		...
		() => {
			set(
				...
				(() => {
					...
					today.setHours(0, 0, 0, 0);
					return Math.floor(
						(today.getTime() - get(dateRange).start.getTime()) / 864e5,
					);
				})(),
			);
		},
		...
		($$anchor2, row) => {
			...
			var node = child(div_3);
			{
				var consequent_1 = ($$anchor3) => {
					const task = /* @__PURE__ */ derived_safe_equal(
						() => (
							deep_read_state(tasks()),
							get(row),
							untrack(() => tasks().find((t) => t.id === get(row).id))
						),
					);
					...
					var node_1 = first_child(fragment);
					{
							...
							template_effect(
								...
								[
									() => (
										get(expanded),
										get(row),
										untrack(() => (get(expanded).has(get(row).id) ? "▾" : "▸"))
									),
								],
							);
						...
						if_block(node_1, ($$render) => {
							if (
								(deep_read_state(get(task)),
								untrack(() => get(task) && get(task).subtasks.length > 0))
							)
								$$render(consequent);
				...
				if_block(node, ($$render) => {
					if ((get(row), untrack(() => !get(row).isSubtask)))
						$$render(consequent_1);
			...
			template_effect(() => {
				classes = set_class(
					div_3,
					1,
					"gantt-left-row svelte-152mm6m",
					null,
					classes,
					{ "subtask-row": get(row).isSubtask },
				);
				set_style(
					div_3,
					`height:40px; padding-left:${(get(row), untrack(() => 8 + get(row).depth * 18)) ?? ""}px`,
				);
				set_attribute(
					span_3,
					"title",
					(get(row), untrack(() => get(row).title)),
				);
				set_text(text_1, (get(row), untrack(() => get(row).title)));
				set_style(
					span_4,
					`background:${(get(row), untrack(() => statusColors[get(row).status] ?? "#888")) ?? ""}`,
				);
			});
			...
			event(
				"keydown",
				span_3,
				(e) => e.key === "Enter" && onOpenTask()(get(row).filePath),
			);
	...
	each(
		...
		($$anchor2, m) => {
			...
			template_effect(() => {
				set_style(
					div_6,
					`width:${(get(m), untrack(() => get(m).span * DAY_WIDTH)) ?? ""}px`,
				);
		...
		($$anchor2, dh) => {
			...
			template_effect(() => {
				classes_1 = set_class(
					div_8,
					1,
					"gantt-day-cell svelte-152mm6m",
					null,
					classes_1,
					{ weekend: get(dh).isWeekend, today: get(dh).isToday },
				);
	...
	var node_2 = child(div_9);
	{
		var consequent_2 = ($$anchor2) => {
			...
			template_effect(() =>
				set_style(
					div_10,
					`left:${get(todayIdx) * DAY_WIDTH + DAY_WIDTH / 2}px; height:${(get(rows), untrack(() => get(rows).length * ROW_HEIGHT)) ?? ""}px`,
				),
			);
		...
		if_block(node_2, ($$render) => {
			if (
				...
				get(dateRange),
				untrack(
					() => get(todayIdx) >= 0 && get(todayIdx) < get(dateRange).days,
				))
			)
				$$render(consequent_2);
		});
		...
		($$anchor2, row) => {
					...
					template_effect(
						() =>
							(classes_2 = set_class(
								div_12,
								1,
								"gantt-grid-cell svelte-152mm6m",
								null,
								classes_2,
								{ weekend: get(dh).isWeekend, today: get(dh).isToday },
							)),
			...
			var node_5 = sibling(node_4, 2);
			{
				var consequent_3 = ($$anchor3) => {
					const bar = /* @__PURE__ */ derived_safe_equal(
						() => (get(row), untrack(() => getBar(get(row)))),
					);
					const barWidth = /* @__PURE__ */ derived_safe_equal(
						() => (
							deep_read_state(get(bar)),
							untrack(
								() => (get(bar).endDay - get(bar).startDay + 1) * DAY_WIDTH,
							)
						),
					);
					var div_13 = root_11();
              ...
              `,
					...
					event("mousedown", div_14, (e) =>
						onBarMouseDown(get(row), "resize-start", e),
					);
					event("mousedown", div_15, (e) =>
						onBarMouseDown(get(row), "resize-end", e),
					);
					event("mousedown", div_13, (e) =>
						onBarMouseDown(get(row), "move", e),
					);
					append($$anchor3, div_13);
				};
				var d_1 = /* @__PURE__ */ user_derived(
					() => (get(row), untrack(() => getBar(get(row)))),
				);
				if_block(node_5, ($$render) => {
			...
			template_effect(() =>
				set_style(
					div_11,
					`height:40px; width:${(get(dateRange), untrack(() => get(dateRange).days * DAY_WIDTH)) ?? ""}px`,
				),
			);
...
function TaskModal($$anchor, $$props) {
	...
	function validate() {
		set(errors, {});
		if (!get(title).trim())
			mutate(errors, (get(errors).title = "Title is required"));
		if (get(startDate) && get(endDate) && get(endDate) < get(startDate)) {
			mutate(
				errors,
				(get(errors).endDate = "End date must be after start date"),
			);
		}
	...
	var node_1 = sibling(input, 2);
	{
		var consequent_1 = ($$anchor2) => {
			...
			template_effect(() =>
				set_text(text_2, (get(errors), untrack(() => get(errors).title))),
			);
			append($$anchor2, span_1);
		};
		if_block(node_1, ($$render) => {
			if ((get(errors), untrack(() => get(errors).title)))
				$$render(consequent_1);
		});
	}
	...
	var node_2 = sibling(input_2, 2);
	{
		var consequent_2 = ($$anchor2) => {
			...
			template_effect(() =>
				set_text(text_3, (get(errors), untrack(() => get(errors).endDate))),
			);
			append($$anchor2, span_2);
		};
		if_block(node_2, ($$render) => {
			if ((get(errors), untrack(() => get(errors).endDate)))
				$$render(consequent_2);
		});
	}
	...
	template_effect(() => {
		...
		classes = set_class(input, 1, "svelte-1gmf2hi", null, classes, {
			error: get(errors).title,
		});
		classes_1 = set_class(input_2, 1, "svelte-1gmf2hi", null, classes_1, {
	...
	bind_value(
		input_1,
		() => get(startDate),
		($$value) => set(startDate, $$value),
	);
	bind_value(
		input_2,
		() => get(endDate),
		($$value) => set(endDate, $$value),
	);
	bind_value(
		input_3,
		() => get(assignee),
		($$value) => set(assignee, $$value),
	);
	bind_value(
		input_4,
		() => get(tags),
...
var root_2 = /* @__PURE__ */ from_html(
	`<span class="no-projects svelte-gjpmyc">No projects found in your projects folder.</span>`,
);
var root_3 = /* @__PURE__ */ from_html(
	`<button class="btn-add svelte-gjpmyc">+ New Task</button>`,
);
var root_4 = /* @__PURE__ */ from_html(
	`<div class="empty-state svelte-gjpmyc"><div class="empty-icon svelte-gjpmyc">📁</div> <p>No project selected. Create a folder inside your configured projects folder to get started.</p></div>`,
);
var root = /* @__PURE__ */ from_html(
	`<div class="project-view svelte-gjpmyc"><div class="topbar svelte-gjpmyc"><div class="project-selector svelte-gjpmyc"><span class="topbar-label svelte-gjpmyc">Project:</span> <!> <!></div> <div class="view-switcher svelte-gjpmyc"><button title="Gantt Chart">📊 Gantt</button> <button title="Kanban Board">🗂 Kanban</button></div> <div class="topbar-actions svelte-gjpmyc"><!> <button class="btn-refresh svelte-gjpmyc" title="Refresh">↺</button></div></div> <div class="view-container svelte-gjpmyc"><!></div></div> <!>`,
	1,
);
function ProjectView($$anchor, $$props) {
	...
	var node_1 = sibling(node, 2);
	{
		...
		if_block(node_1, ($$render) => {
			if ((deep_read_state(projects()), untrack(() => projects().length === 0)))
				$$render(consequent);
		});
	}
	var div_3 = sibling(div_2, 2);
		...
		var alternate = ($$anchor2) => {
			KanbanBoard($$anchor2, {
				...
				onStatusChange: (taskId, newStatus) =>
					onStatusChange()(get(currentProject).folderPath, taskId, newStatus),
			});
		};
		if_block(node_3, ($$render) => {
	...
	template_effect(() => {
		classes_1 = set_class(
			button_1,
			1,
			"view-btn svelte-gjpmyc",
			null,
			classes_1,
			{ active: get(viewMode) === "gantt" },
		);
		classes_2 = set_class(
			button_2,
			1,
...
class GanttPlugin extends obsidian.Plugin {
	...
	async onload() {
		...
		this.addRibbonIcon("refresh-cw", "Sync tasks", async () => {
			...
			if (!syncBaseUrl || !syncEmail || !syncPassword) {
				new obsidian.Notice(
					"⚠️  Configure sync credentials in Settings first.",
				);
				return;
			}
			...
			try {
				...
				await svc.login();
				const count = await svc.syncAll(this.app, this.settings.projectsFolder);
				new obsidian.Notice(`✅ Sync complete — ${count} task(s) updated.`);
...
class GanttSyncService {
	...
	async pullSince(since) {
		...
		const data = await this._fetch(`/tasks/pull${qs}`);
		return data.tasks ?? data ?? [];
	}

	async syncAll(app, projectsFolder) {
		// Push all local tasks
		...
		for (const project of projects) {
			for (const task of project.tasks) {
				try {
					...
					console.warn(`[Gantt] pushTask failed for ${task.id}:`, e.message);
				}
			}
		}

		// Pull remote delta
		const remoteTasks = await this.pullSince(this.lastPullAt);
		...
		for (const rt of remoteTasks) {
			try {
				...
				const content = generateTaskFrontmatter({
					id: rt.id,
					title: rt.title,
					status: rt.status,
				...
				console.warn(
					`[Gantt] Failed to write remote task ${rt.id}:`,
					e.message,
				);
			}
		}
