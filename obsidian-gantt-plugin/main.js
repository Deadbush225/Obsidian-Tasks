...
class GanttSyncService {
  ...
  async syncAll(app, projectsFolder) {
    ...
    if (localTasks.length > 0) {
      ...
      const payload = localTasks.map((t, i) => ({
        id: t.id,
        title: t.title,
        status: t.status,
        priority: t.priority,
        ...
        rawFrontmatter: rawContents[i] ?? null
        // ← full .md content so server stores it verbatim
      }));
  ...
  async forcePushTasks(app, projectsFolder, conflictTasks) {
    ...
    const overrideTasks = await Promise.all(conflictTasks.map(async (rt) => {
      const local = localById.get(rt.id);
      ...
      if (local) {
        try {
          ...
          if (file) {
            ...
            rawFrontmatter = rawFrontmatter.replace(/^(updated_at:\s*).*$/m, `$1${winningTs}`);
...
`);
      ...
      return {
        ...
        rawFrontmatter
        // verbatim file so other clients get the full content
      };
    }));
...
`);
      ...
      const existingFile2 = app.vault.getFileByPath(filePath);
      if (existingFile2) {
        const oldContent = await app.vault.read(existingFile2);
...
var root_9 = /* @__PURE__ */ from_html(`<div class="empty-state svelte-gjpmyc"><div class="empty-icon svelte-gjpmyc">📁</div> <p>No project selected. Create a folder inside your configured projects
					folder to get started.</p></div>`);
