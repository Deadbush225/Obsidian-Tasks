...
async function archiveTask(app, taskFilePath, taskId, projectFolder, isSubtask) {
  ...
  if (file) {
    ...
    const finalDest = app.vault.getFileByPath(destPath) ? `${archiveDir}/${taskId}-${file.name}` : destPath;
  ...
  if (!isSubtask) {
    ...
    if (subFolder) {
      ...
      for (const child2 of children) {
        ...
        if (!childFile.extension) continue;
        if (childFile.extension === "md") {
          const childDest = app.vault.getFileByPath(`${archiveSubDir}/${childFile.name}`) ? `${archiveSubDir}/${childFile.basename}-${taskId}.${childFile.extension}` : `${archiveSubDir}/${childFile.name}`;
