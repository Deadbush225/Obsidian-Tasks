...
class GanttSettingTab extends obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new obsidian.Setting(containerEl).setName("Server URL").setDesc("Base URL of your sync backend, e.g. https://yourname.heliohost.us/obgantt/backend").addText(
      (text) => text.setPlaceholder("https://…/backend").setValue(this.plugin.settings.syncBaseUrl).onChange(async (value) => {
    ...
    new obsidian.Setting(containerEl).setName("Email").setDesc("Account email address for the sync server.").addText(
      (text) => text.setPlaceholder("you@example.com").setValue(this.plugin.settings.syncEmail).onChange(async (value) => {
        this.plugin.settings.syncEmail = value.trim();
        await this.plugin.saveSettings();
      })
    );
    new obsidian.Setting(containerEl).setName("Password").setDesc("Account password. Stored in Obsidian plugin data.").addText((text) => {
      text.inputEl.type = "password";
      text.setPlaceholder("••••••••").setValue(this.plugin.settings.syncPassword).onChange(async (value) => {
